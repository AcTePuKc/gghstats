...
Catalog (gghstats repo / gghstats featured — local SQLite, no GitHub token):
  ...
  repo ls                 List pinned repos
	  repo report ls          List visibility/policy (--json, --visibility, --policy)
...
Server env vars (serve):
  ...
  GGHSTATS_API_TOKEN           Protect /api/repos and /api/v1 dogfood routes (and badges when GGHSTATS_BADGE_PUBLIC=false)
  GGHSTATS_API_ONLY            Skip HTML + SEO routes; JSON/probes only (default false)
  GGHSTATS_CORS_ORIGINS        Allowed Origin list for JSON CORS (empty = *)
  GGHSTATS_CSP                 report-only (default) or enforce (when HEAD_HTML empty)
