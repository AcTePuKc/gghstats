...
Use **`GGHSTATS_DB=/var/db/gghstats/gghstats.db`** (or another persistent path) on servers. The binary default when unset is the platform config dir (Linux ~/.config/gghstats/, macOS ~/Library/Application Support/gghstats/).
